/* eslint-disable jsx-a11y/anchor-is-valid */
import React from 'react'
import silver from "../Assets/images/silver2.jpeg"
import gold from "../Assets/images/yello2.jpeg"
import brass from "../Assets/images/brass2.jpeg"
const Pricing = () => {
  return (
    <>
      <div class="bg-white py-6 sm:py-8 lg:py-12">
        <div class="mx-auto max-w-screen-xl px-4 md:px-8">
          <h2 class="mb-4 text-center text-2xl font-bold text-gray-800 md:mb-8 lg:text-3xl xl:mb-12">COMBO OFFERS</h2>

          

          <div class="mb-6 grid gap-6 sm:grid-cols-2 md:mb-8 lg:grid-cols-3 lg:gap-8">

            <div class="flex flex-col overflow-hidden rounded-lg border sm:mt-8">
              <div class="h-2 bg-yellow-800"></div>

              <div class="flex flex-1 flex-col p-6 pt-8">
                <div class="mb-12">
                  <div class="mb-4 text-center text-2xl font-bold text-gray-800">Explorer </div>
                  <div style={{ display: 'flex', justifyContent: 'center' }}>
                    <img alt="hero" src={brass} style={{ height: '150px' }} />
                  </div>

                  <div class="space-y-4">

                    <div class="flex items-center gap-2">
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 shrink-0 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                      </svg>

                      <span class="text-gray-600">Pro Show</span>
                    </div>

                    <div class="flex items-center gap-2">
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 shrink-0 text-red-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                      </svg>

                      <span class="text-gray-600">Technical Events</span>
                    </div>

                    <div class="flex items-center gap-2">
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 shrink-0 text-red-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                      </svg>

                      <span class="text-gray-600">Non Technical Events</span>
                    </div>



                  </div>
                </div>

                <div class="mt-auto">
                  <a class="block rounded-lg bg-gray-200 px-8 py-3 text-center text-sm font-semibold text-red-500 outline-none ring-indigo-300 transition duration-100 hover:bg-gray-300 focus-visible:ring active:text-gray-700 md:text-base flex justify-center"><div class="text-red-500 line-through text-xl pe-2 mt-1.5">₹500</div>
                    <div class="text-black text-3xl">₹300</div></a>
                </div>``
              </div>
            </div>

            <div class="flex flex-col overflow-hidden rounded-lg border-2 border-gray-500">
              <div class="bg-gray-400 py-2 text-center text-sm font-semibold uppercase tracking-widest text-white">Popular choice</div>

              <div class="flex flex-1 flex-col p-6 pt-8">
                <div class="mb-12">
                  <div class="mb-2 text-center text-2xl font-bold text-gray-800">Champion</div>
                  <div style={{ display: 'flex', justifyContent: 'center' }}>
                    <img alt="hero" src={silver} style={{ height: '150px' }} />
                  </div>

                  <div class="space-y-4">

                    <div class="flex items-center gap-2">
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 shrink-0 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                      </svg>

                      <span class="text-gray-600">Pro Show</span>
                    </div>

                    <div class="flex items-center gap-2">
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 shrink-0 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                      </svg>

                      <span class="text-gray-600">Technical Event <strong>(Any One)</strong></span>
                    </div>
                    <p class="mb-6 text-red-500 sm:text-lg md:mb-8">
                      <strong>NOTE : Except Paper Presentation.</strong>
                    </p>

                    <div class="flex items-center gap-2">
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 shrink-0 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                      </svg>

                      <span class="text-gray-600">Non Technical Event <strong>(Any One)</strong></span>
                    </div>
                    <br />
                    

                  </div>
                </div>

                <div class="mt-auto">
                  <a class="block rounded-lg bg-gray-200 px-8 py-3 text-center text-sm font-semibold text-red-500 outline-none ring-indigo-300 transition duration-100 hover:bg-gray-300 focus-visible:ring active:text-gray-700 md:text-base flex justify-center"><div class="text-red-500 line-through text-xl pe-2 mt-1.5">₹700</div>
                    <div class="text-black text-3xl">₹400</div></a>
                </div>
              </div>
            </div>



            <div class="flex flex-col overflow-hidden rounded-lg border lg:mt-8">
              <div class="h-2 bg-yellow-500"></div>

              <div class="flex flex-1 flex-col p-6 pt-8">
                <div class="mb-12">
                  <div class="mb-2 text-center text-2xl font-bold text-gray-800">Achiever</div>
                  <div style={{ display: 'flex', justifyContent: 'center' }}>
                    <img alt="hero" src={gold} style={{ height: '150px' }} />
                  </div>

                  <div class="space-y-4">

                    <div class="flex items-center gap-2">
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 shrink-0 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                      </svg>

                      <span class="text-gray-600">Pro Show</span>
                    </div>

                    <div class="flex items-center gap-2">
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 shrink-0 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                      </svg>

                      <span class="text-gray-600">Technical Event <strong>(Any Two)</strong></span>
                    </div>

                    <div class="flex items-center gap-2">
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 shrink-0 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                      </svg>

                      <span class="text-gray-600">Non Technical Event <strong>(Any Two)</strong></span>
                    </div>
                  </div>
                </div>

                <div class="mt-auto">
                  <a class="block rounded-lg bg-gray-200 px-8 py-3 text-center text-sm font-semibold text-red-500 outline-none ring-indigo-300 transition duration-100 hover:bg-gray-300 focus-visible:ring active:text-gray-700 md:text-base flex justify-center"><div class="text-red-500 line-through text-xl pe-2 mt-1.5">₹1000</div>
                    <div class="text-black text-3xl">₹500</div></a>
                </div>
              </div>
            </div>

          </div>
        </div>
      </div>

    </>
  )
}

export default Pricing